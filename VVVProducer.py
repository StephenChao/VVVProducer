...
class VVVProducer(Module):
  ...
  def analyze(self, event):
    ...
    # print("Electron's amount is",nLooseEle)
    # print("Muon's amount is",nLooseMu)
    ...
    # for just one lep!!!Otherwise it will out range!!!
    # Ignnore the glepton for now.
    fatjet1,fatjet2,fatjet3=Process_fatJets(self,event)
    Process_1Lepton_Jets(self,event)

    if not (fatjet1.M()>40 and fatjet1.Pt()>450): return False
    
    Nj8=(fatjet1.M()>40 and fatjet1.Pt()>450)+(fatjet2.M()>0 and fatjet2.Pt()>200)+(fatjet3.M()>0 and fatjet3.Pt()>200)
    if Nj8==0: return False
    self.out.fillBranch("Nj8", Nj8)
    # if(nLooseMu+nLooseEle>4):
    #     if nLooseMu==1:
